...
function gs_wp_kses(  $content  ) {
    ...
    $allowed_tags = array_merge_recursive( $allowed_tags, [
        ...
        'input'  => array_merge( $input_common_atts, [
            'type'    => true,
            'checked' => true,
        ] ),
        'option' => [
...
function terms_hierarchically(
    ...
    foreach ( $cats as $i => $cat ) {
        if ( in_array( $cat->term_id, $exclude_group ) ) {
            continue;
        }
...
function gstm_read_more(  $is_popup_enabled = false, $shortcode_id = null  ) {
    ...
    if ( $is_popup_enabled && $shortcode_id ) {
        return sprintf(
            '<a class="gstm-popup--link" data-mfp-src="#gstm_popup_%s_%s" aria-label="Testimonial Details Link" href="#">%s</a>',
            get_the_ID(),
            $shortcode_id,
            esc_html( $gstm_read_more )
        );
    }
}
...
function gs_get_terms(  $term_name, $idsOnly = false  ) {
    ...
    if ( $idsOnly ) {
        return wp_list_pluck( $_terms, 'term_id' );
    }
    $terms = [];
